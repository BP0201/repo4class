...
class User {
  ...
  static async updateLoginTimestamp(username) { 
    const results = await db.query(`UPDATE users SET last_login_at = $1 WHERE username = $2 RETURNING username`,
    ["2022-12-20 20:45:27", username]);

    if (results.rows[0].length === 0) {
      throw new ExpressError("User could not be found", 404)
    }
  }
