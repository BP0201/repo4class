...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        if (results.rows.length === 0) {
            throw new ExpressError(`Couldn't find invoice with id: ${id}`, 404)
        }
...
router.put('/:id', async (req, res, next) => {
    try {
        ...
        const results = await db.query(`UPDATE invoices SET amt=$1, paid=$3, paid_date=$4 WHERE id=$2 RETURNING *`, [amt, id, paid, paid_date])
        if (results.rows.length === 0) {
